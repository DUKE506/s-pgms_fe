...
**백엔드 연동 작업을 할 때는 반드시 [`.claude/loop-backend/`](.claude/loop-backend/)를 먼저 확인하고 그 절차를 따르세요** (loop-screens와는 별도 loop — 완료 기준이 "스크린샷 승인"이 아니라 "API 연동 정확성 + 회귀 없음"이라 분리함, 2026-09-01 결정). 읽는 순서는 `README.md` → `TASK.md`(정책·판단 기준·정지조건) → `LOOP_INSTRUCTIONS.md`(한 iteration 실행 절차 = 3국면·3게이트) → `PROGRESS.md`(진행 상태) → `CARRYOVER.md`(재검증·회신 대기). 템플릿은 `templates/`(작업계획서·완료보고서), 상세 지침은 `guides/`(프로브 실무·섹션 종료 절차), 로컬 전용 파일은 `local/`(gitignore — 테스트 계정·프로브 스크립트). **개발은 화면 단위(1 iteration = PROGRESS 표 1행), 백엔드 요청은 섹션 단위** — 자세한 건 `TASK.md`. "무엇을 연동하는가"의 기록은 `docs/backend-integration/` — `matrix.md`(역할×화면×API + 의존성 순서), `findings.md`(요청·질문·블로커·제외 레지스터), `responses/`(실측 응답), `requests/`(백엔드로 나간 요청서). 실제 DB 스키마는 `docs/db-dump/`, 스웨거 원본은 `docs/api-swagger.json`.
