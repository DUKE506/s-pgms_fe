...
(`docs/backend-integration-process.md` 원칙 4).
...
- [ ] **그룹 A — [경찰서] 피전 경호관리 섹션** (경호목록→접수→상세→수정)
      ...
      - [~] 경호 상세 — 조회 `GET Deploy/Police/W/GetDeployDetail`, 접수취소·경호취소
        ...
        표시 보류(△).
      - [ ] 배치요구서 수정 — `PUT UpdateDeployRequest`. prefill 필드 부족(성별·생년월일·
        직업 등) 어디서 받을지 먼저 확인. **여기까지가 피전 경호관리 섹션** — 종료 시
        issues(#5·#6 등)/exclusions 정리해 백엔드 일괄 요청.
