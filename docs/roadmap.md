...
- [ ] [내부] `assignee`(이름 문자열) → `assigneeId`(계정 id 참조)로 리팩터.
...
- [ ] [본사] 관리자 계정 관리 (신규, 목업 없음) — 메뉴명 "관리자"
      (`/admin/managers`, `COMPANY_ALL` 가드). 시스템관리자는 본인+운영관리자+
      본부관리자 전체 조회, 운영관리자는 본인+본부관리자 조회(본부관리자
      로그인 시엔 메뉴 자체 비노출). 본부관리자는 본부당 계정 1개 고정 운영
      전제. 할당 경호건 수정(해당 관리자 배정 건을 다른 담당자로 재배정 —
      기존 `assignManager` 재사용)은 대상 무관 공통 액션. 게스트 계정 관리
      (화면 9/10)와 동일한 "목록 + 모달" 패턴으로 별도 상세페이지 없이 구성
      (2026-08-31 결정 — 액션들이 다 모달 규모, 본부관리자당 배정 건수도
      많지 않아 목록 화면 규모로 충분하다고 판단, 필요해지면 그때 분리).

  **정보수정(이름/연락처)·비밀번호 초기화 권한 매트릭스** (2026-08-31 확정):
  | 실행자 \ 대상 | 시스템관리자 | 운영관리자 | 본부관리자 |
  |---|---|---|---|
  | 시스템관리자 | 정보수정·초기화 O(본인) | 정보수정·초기화 O | 정보수정·초기화 O |
  | 운영관리자 | 접근 불가 | 정보수정·초기화 O(본인만) | **비밀번호 초기화만 O, 정보수정은 불가** |

  시스템관리자 본인 행도 본인이 정보수정·비밀번호초기화 가능(2026-08-31 확인).
  본부관리자는 이 메뉴 자체에 접근 불가이므로 본인 정보수정/비밀번호 변경은
  별도인 [공통] 프로필 화면(위 Phase 3.5 후속 항목) 몫.
...
- [ ] [본사] 비밀번호 초기화 후 최초 로그인 강제 변경 플로우 — 관리자 계정
      관리에서 비밀번호를 초기화해도 지금은 그 값을 그대로 계속 쓸 수 있음.
      게스트 계정 발급 때도 같은 이유로 스코프 아웃했던 항목(로드맵 Phase 3
      항목3 참고)과 동일한 성격이라 함께 후속으로 미룸(2026-08-31).
