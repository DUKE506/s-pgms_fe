...
- [ ] [본사] 연장요청/단축요청 승인 화면 — 목업 anchor 없음(원본 목업에도 미설계, `SecurityCaseTabs`의 "연장요청"/"단축요청" 탭 자리만 존재). Phase 1 항목 5(경찰 경호 상세)에서 경찰이 연장/단축을 "요청"만 제출하고 즉시 반영은 안 되게 만들었는데, 이 승인 화면이 없으면 요청이 처리될 곳이 없어 s3/s6d 때와 같은 문제가 재발함 — 다음 순서로 최우선 진행 예정(2026-08-25)
