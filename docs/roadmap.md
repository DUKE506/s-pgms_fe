...
- [x] [공통] 경찰/본사 로그인 화면 통합 — 로그인 연동(위 1번 항목) 중 실제 백엔드가
      ...
      변경 없음(로그인 라우트만 없앤 것).
- [ ] [공통] 목록/이력 화면 필터·페이지네이션을 URL 쿼리스트링 + 서버 파라미터 기반으로
      전환 — 지금은 대부분 화면이 전량을 받아 클라이언트에서 필터링하고 페이지네이션
      UI 자체가 없다. 2026-09-18 대화에서 화면별 서버/클라이언트 처리 방식과 하이브리드
      페이지네이션(xl 이상 번호형·xl 미만 더보기)까지 구체적으로 확정 — 상세는
      `architecture.md`의 "상태관리" 섹션. 아직 구현 착수 전(계획 단계).
