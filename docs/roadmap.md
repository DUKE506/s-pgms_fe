...
- [x] ~~[공통] 프로필 화면~~ — **폐기(2026-09-11, 사용자 결정)**: 본인 계정 비밀번호
      변경 기능만 넣으려던 것이었으나 필요 없다고 확정, 신규 개발 안 함.
- [x] [본사] 연장/단축요청 미확인 표시 — **완료(2026-09-11)**. 경찰이 연장·단축을
      요청해도 본사 관리자가 연장요청/단축요청 탭에 직접 들어가지 않으면 알 방법이
      없다는 문제(2026-08-28, Phase 3.5 항목2 통합 테스트 중 발견) — 경호상세
      페이지 상태뱃지 옆에 안내 문구로 반영(경호목록은 범위 밖, 상세만). 실제로는
      `GetGuardCaseDetail`/`GetGuardCaseList`의 statusName이 "경호중" 그대로라 연장/
      단축 요청 목록 API(`GetExtendRequestList`/`GetShortenRequestList`)에 caseSeq가
      있는지로 판정(`findPendingPeriodRequestType`). 디자인은 사전미팅 삭제 버튼과
      같은 destructive 배색(연한 빨강 테두리·배경)의 텍스트 배지. 실백엔드 확인 완료,
      커밋 `f4e994c`.
...
- [x] [본청/지역청/경찰서] 홈 대시보드(`/dashboard`, 공용) — 원본 목업(`s1` `s1m` `s2` `s2m`,
      조직 트리 드릴다운)은 폐기하고 `docs/mobile-ui/홈 대시보드 (모바일).dc.html`로
      전면 교체(2026-09-14 결정, 모바일 헤더 리디자인 작업 중 발견). 집계 API가 아직
      없어 정적 더미 데이터로만 채움 — 실 연동은 Phase 5(loop-backend) 몫.
- [ ] [본사] 본사 전체 대시보드 — `s6` (이번 목업엔 본사 버전이 없어 범위 밖, 별도 논의 필요. **2026-09-15**: 당분간 개발 안 할 것 같다는 사용자 판단으로 사이드바 메뉴에서 제외, 기본 랜딩도 경호관리로 변경 — 착수는 여전히 미정, 재개 시 이 항목 갱신)
...
- [x] [공통] 경찰/본사 로그인 화면 통합 — 로그인 연동(위 1번 항목) 중 실제 백엔드가
      로그인을 원래 하나로 취급한다는 게 확인됨(`Login` 응답의 `code` 필드가
      `100+codeSeq`라 역할까지 이미 구분 가능, 두 화면이 이미 내부적으로 같은 `login()`을
      공유하고 있어 접근 제어상 분리 효과도 없는 상태). 2026-09-01엔 제품/UX 결정이라
      loop-backend 범위 밖으로 분리했었으나, **2026-09-11 사용자 결정으로 실행** —
      `PoliceLoginPage`/`CompanyLoginPage`를 `LoginPage.tsx` 하나로 통합(제목 "로그인"),
      로그인 성공 후 이동은 기존과 동일하게 `getDefaultRouteForRole(role)`이 담당. `/admin`
      로그인 라우트 제거 — `ProtectedRoute`의 미인증 리다이렉트도 항상 `/`로 단순화,
      `CompanyAppShell` 로그아웃도 `/`로 이동. 본사 내부 화면 경로(`/admin/dashboard` 등)는
      변경 없음(로그인 라우트만 없앤 것).
- [x] [공통] 목록/이력 화면 필터·페이지네이션을 URL 쿼리스트링 + 서버 파라미터
      기반으로 전환 — **완료(2026-09-18)**. 서버 지원 화면(본사 경호목록·이력조회,
      경찰 이력조회·경호목록)은 검색·기간·상태·지역청→경찰서를 서버 파라미터로,
      하이브리드 페이지네이션(xl 이상 번호형·xl 미만 더보기)까지 적용. 클라이언트
      전용 화면(근무자·관리자 3탭·배치요청·연장/단축 요청, 게스트 계정)은 검색어만
      URL 동기화. 상세는 `architecture.md`의 "상태관리" 섹션.
