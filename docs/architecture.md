...
- 목록/이력 조회 화면의 필터(상태·기간·조직·검색 등): 화면별로 서버 파라미터
  ...
  **진행 상태(2026-09-18) — 전 배치 완료**: 공용 인프라(`shared/hooks/
  useUrlParam.ts`, `shared/hooks/useIsDesktop.ts`, `shared/components/
  HybridPagination.tsx`) + 파일럿(본사 경호목록) + 배치 A(서버 지원 3화면: 경찰
  이력조회·본사 이력조회·경찰 경호목록) + 배치 B(클라이언트 전용 7화면: 본사
  근무자·관리자 3탭[본사/경찰/게스트]·배치요청·연장/단축 요청, 경찰 게스트 계정)
  전부 완료 — 실백엔드 브라우저 검증까지 끝남.

  서버 지원 화면(검색·기간·상태·지역청→경찰서 캐스케이드)은 URL 쿼리를 그대로
  API 파라미터로 전달하고, 페이지네이션이 있는 화면은 페이지 이동·새로고침 유지가
  확인됐다(경찰 경호목록처럼 페이지네이션이 없는 화면은 상태를 여전히 클라이언트
  필터로 처리). 지역청/경찰서 옵션(id+이름)은 `features/police/api/
  accountManagement.ts::listOrgTree`(계정관리 화면의 `GetPoliceUserList` 조회를
  ...
  클라이언트 전용 화면은 서버 API에 필터 파라미터 자체가 없어(또는 배치요청/연장·
  단축 요청처럼 서버 전송 여부가 아직 미확정이라) 검색어만 URL 쿼리(`?q=`)에
  동기화하고 필터링 자체는 그대로 클라이언트에서 한다 — 새로고침해도 검색어가
  유지되는 것만 개선한 것이며 서버 호출은 무변경.

  **검색창은 엔터/검색버튼을 눌러야 커밋된다**(2026-09-18, 사용자 리포트로 수정) —
  처음엔 매 키 입력마다 URL(`?q=`)을 갱신했는데, 그 리렌더가 한글 조합(IME) 중간에
  끼어들어 "이동희"가 "ㅇㅣㄷㅗㅇ"으로 분해되는 문제가 있었다. `shared/hooks/
  useSearchDraft.ts`(입력 중 값을 로컬 state로 분리) + `shared/hooks/
  useUrlSearchInput.ts`(단일 필드 화면용, useUrlParam과 조합) + `shared/
  components/SearchInput.tsx`(엔터/검색아이콘 클릭 시 커밋)로 모든 검색창(서버·
  클라이언트 화면 공통 11개)을 통일했다. 필터가 여러 개라 커밋 시 다른 파라미터도
  같이 다뤄야 하는 화면(본사 경호목록·이력조회, 경찰 이력조회)은 `useSearchDraft`만
  직접 쓰고 나머지(상태·기간·지역청 등)는 기존처럼 즉시 반영한다.

  **기간 필터는 값이 있을 때만 필드별 해제(X) 버튼이 뜬다**(2026-09-18, 화면
  전체를 되돌리는 "초기화" 버튼 대신 각 필드를 독립적으로 지울 수 있게 — 다른
  필터들도 전부 "전체"로 개별 리셋 가능한 것과 통일) — `shared/components/
  DateField.tsx`의 캘린더 variant에 추가.
