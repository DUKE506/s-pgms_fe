...
- 목록/이력 조회 화면의 필터(상태·기간·조직·검색 등): 지금은 `useState`로만 관리해
  새로고침하면 초기화되고, 페이지네이션 UI 자체가 없다(서버 `pageSize` 상한까지
  전량을 순회해 모은 뒤 클라이언트에서 필터링). **추후 계획**(2026-08-27 방향 결정 →
  2026-09-18 화면별로 구체화, 아직 구현 착수 전) — 필터 상태를 URL 쿼리스트링으로
  옮겨 새로고침해도 유지되게 하고, 서버가 지원하는 화면은 그 쿼리를 그대로 API
  요청 파라미터로 전달한다.
