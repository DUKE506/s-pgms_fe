...
- 목록/이력 조회 화면의 필터(상태·기간·조직·검색 등): 지금은 `useState`로만 관리해
  새로고침하면 초기화된다. **추후 계획**(아직 미적용) — 실제 백엔드 연동 시점에
  필터 상태를 URL 쿼리스트링으로 옮겨서 새로고침해도 유지되게 하고, 그 쿼리스트링을
  그대로 백엔드 API 요청 파라미터로 전달하는 방향으로 전환할 예정(2026-08-27 결정,
  당장 구현 대상 아님 — 지금 화면들의 `useState` 필터를 이 방향에 맞춰 미리
  리팩터링해두지 않아도 됨).
