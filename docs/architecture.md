...
- 목록/이력 조회 화면의 필터(상태·기간·조직·검색 등): 화면별로 서버 파라미터
  ...
  **파일럿(본사 경호목록) 완료** — 실백엔드(StecM1) 브라우저 검증까지 끝남(검색·
  지역청→경찰서 캐스케이드·상태 필터가 URL 쿼리와 서버 파라미터로 반영, 데스크톱
  번호형 페이지네이션 페이지 이동·새로고침 유지, 모바일 더보기 누적 전부 확인).
  지역청/경찰서 옵션(id+이름)은 `features/police/api/accountManagement.ts::
  listOrgTree`(계정관리 화면의 `GetPoliceUserList` 조회를 공유, flatten 방식만
  다름)에서 가져온다. 나머지 3화면(서버 지원)과 클라이언트 전용 4화면은 아직
  미착수.
