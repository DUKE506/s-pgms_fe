...
// 근무자 마스터 CRUD(본사 admin)와 근무자 상세의 근무 이력(GetGuardSchedule)은
// 실제 백엔드(Guard/Stec/W/*)로 연동 완료 — 그 검증은 테스트 전용 더블
// (mocks/handlers/guard.ts)에서 한다. 여기 남은 GET /api/workers 하나는 경호
// 상세(#9)·이력 상세(#13)가 근무자 이름/연락처 조인용으로 아직 읽는 임시 경로
// (features/company/api/workers.ts listCaseJoinWorkers). 그 화면들이 각자
// iteration에서 정리한다.
