...
function SecurityCaseNewPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      showToast('배치요구서 등록에 실패했습니다', 'error')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-5 p-4 pb-24 sm:p-8 sm:pb-24 xl:pb-8">
      <div>
        <p className="text-xs text-muted-foreground">경호목록 / 신규 접수</p>
        <h1 className="mt-1 text-xl font-bold text-foreground">신규 접수 · 배치요구서 작성</h1>
        <p className="mt-1 text-xs text-muted-foreground">
          배치요구서를 작성합니다. 모든 항목 입력 후 등록해주세요.
        </p>
      </div>
