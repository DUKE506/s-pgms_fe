...
function GenderSplitBar({ data, total }: GenderSplitBarProps) {
  // total이 0(선택한 조직 범위에 건이 하나도 없음)이면 나눗셈이 NaN%가 된다 —
  // 실 데이터 연동 후 빈 조직을 고르면 실제로 발생(2026-09-16 브라우저 검증 중 발견).
      ...
      <div className="flex items-center justify-between text-[13px] font-medium text-foreground/80">
        {data.map((entry) => (
          <span key={entry.label} className="inline-flex items-center gap-1.5">
            ...
            {entry.label} {entry.count}건 ({pct(entry.count)}%)
          </span>
        ))}
      </div>
      <div className="flex h-2 overflow-hidden rounded-full bg-muted">
        {data.map((entry) => (
          <div key={entry.label} style={{ width: `${pct(entry.count)}%`, backgroundColor: entry.color }} />
        ))}
      </div>
    </div>
  )
