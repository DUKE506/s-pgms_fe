...
| 3 | [본사] 배정 경호건 상세 | `s7a` `s7c` `s7d` `s7e` `s7` `s7b` | 완료 | `3300a9c` `6abb74a` | 워크플로우 수정 다수 반영 후 완료. ~~미해결: 상세 페이지에 상태 가드가 없어 URL을 직접 알면 접수 상태에서도 기본정보 등록이 가능한 우회 경로가 남아있음~~ — **해소 확인(2026-09-11, loop-backend URL 스코프 테스트)**: `GuardCase`(경호건)는 배정 시점에만 생성돼 접수 상태엔 `caseSeq` 자체가 없어 이 화면(`/admin/security-cases/:caseSeq`)으로 도달할 경로가 애초에 없음(아키텍처 분리, Deploy vs GuardCase). 피전 쪽 URL 직접 접근도 접수 상태에서 "접수취소"만 노출되는 것 확인(CARRYOVER-backend C절) |
...
| # | 항목 | 목업 anchor | 상태 | 커밋 | 비고 |
|---|---|---|---|---|---|
| 1 | [본청/지역청/경찰서] 홈 대시보드 | (신규, `docs/mobile-ui/홈 대시보드 (모바일).dc.html`) | 완료 | `44928d7` | 원본 목업(`s1` `s1m` `s2` `s2m`, 조직 트리 드릴다운)은 폐기하고 새 목업으로 전면 교체(2026-09-14 사용자 결정) — 자세한 배경은 아래 iteration 로그 참고. 집계 API 미착수라 정적 더미 데이터. |
| 2 | [본사] 본사 전체 대시보드 | `s6` | 대기 | | 이번 목업엔 본사 버전이 없어 범위 밖, 착수 전 별도 논의 필요. **2026-09-15**: 당분간 개발 안 할 것 같다는 사용자 판단으로 사이드바 메뉴 제외 + 기본 랜딩도 경호관리로 변경(`CompanyAppShell.tsx`/`defaultRoute.ts`) — 착수 자체는 여전히 미정 |
...
- 2026-09-14: 사용자가 `docs/mobile-ui/`에 화면 3개(경호목록·경호상세·홈
  대시보드) 모바일 헤더 목업(html, 디자인 캔버스 산출물)을 추가하고 적용
  가능성 분석을 요청 — 기존 구현과 대조한 결과 상태색·StatusBadge·
  StatusStepper·하단 플로팅 nav 등 핵심은 이미 수렴돼 있었고, 가장 큰
  구조적 차이는 전역 상단 헤더(로고+계정 드롭다운)가 목업엔 아예 없다는
  점이었음. 논의 끝에 결정: (1) 전역 헤더 폐기, (2) 로그아웃은 하단
  플로팅 nav 우측 설정 아이콘 → 전용 페이지(팝오버 아님)로, (3) 목록형/
  상세형 화면은 각각 경호목록/경호상세 목업 패턴을 전체 화면에 일반화
  적용, (4) 사이드바 항목 개수 차이는 무시, (5) 필터칩 모양은 기존 유지,
  (6) 먼저 대시보드·경호목록·경호상세 3화면만 파일럿 적용 후 확정되면
  전체 롤아웃. 착수 전 발견: `/dashboard`가 가리키던 원본 목업(Screen
  1/1m·2/2m, 조직 트리 드릴다운)과 새 홈 대시보드 목업이 완전히 다른
  설계라 사용자에게 재확인 — **원본은 폐기, 새 목업으로 전면 교체**하기로
  결정.

  구현(커밋 `44928d7`): `MobileHeader.tsx` 삭제, `PoliceAppShell`/
  `CompanyAppShell`에서 제거. `Sidebar.tsx` 하단 nav에 설정(Settings)
  아이콘 추가 — 처음엔 DropdownMenu 팝오버로 구현했으나 사용자가 페이지
  이동 방식으로 변경 요청해 `settingsHref` prop(`/settings`·
  `/admin/settings`)으로 재구현. 신규 `shared/pages/SettingsPage.tsx`
  (상단 사용자정보+하단 로그아웃 버튼, 경찰/본사 공용) +
  `features/auth/lib/useLogout.ts`(세 곳 중복 로그아웃 절차 통합) +
  라우트 2개 추가. 경찰 `SecurityCaseListPage`: 제목 옆 모바일 전용 "+"
  아이콘 버튼 신설, 검색 인풋+데스크톱 "신규 접수" 버튼은 `xl` 이상만
  노출(모바일 검색 기능 자체 제거 — 목업 기준, 사용자 확인 후 유지).
  경찰 `SecurityCaseDetailPage`: breadcrumb 줄+제목/뱃지 줄을 모바일에서만
  `gap-3`으로 묶음(`xl:contents`로 데스크톱 무변경). 신규
  `features/police/pages/DashboardPage.tsx`(`/dashboard`, 본청/지역청/
  경찰서 공용) — 히어로(인사말+월선택pill 장식용+KPI)+상태별 현황+
  지역별 순위+이번달 신규접수+상태별 비율 도넛, 전부 목업 실측값 그대로인
  정적 상수. 본사 쪽 목록/상세/대시보드는 이번 파일럿 범위 밖.

  회귀 1건 발견/수정: 새 "+" 버튼과 기존 "신규 접수" 텍스트 버튼이 접근성
  이름이 같아 `SecurityCaseListPage.test.tsx`가 두 개를 다 찾음 —
  `ActionButtons` 모바일/데스크톱 중복 때와 같은 방식(`getAllByRole(...)[0]`)
  으로 보정. `DashboardPage.test.tsx` 신규(본청/경찰서 계정 렌더링, 정적
  수치 노출 확인).

  검증: build/lint 통과, `npm run test` 147/147(신규 2건 포함) 통과. 이
  세션 샌드박스는 실백엔드(`.env.local`의 `API_PROXY_TARGET`, 내부망 IP)
  접근이 안 돼 브라우저 로그인 자체가 막혀(원본 main 코드로도 동일 재현
  확인 — 이번 변경과 무관한 환경 제약) 매 단계 스크린샷은 못 찍었고,
  사용자가 본인 환경(`npm run dev`)에서 4단계 전부 직접 확인 후 승인.

  **다음**: 사용자 승인 완료, 4단계(전체 롤아웃 — 목록형: 이력/게스트/
  배치요청/연장·단축요청/근무자/관리자계정, 상세형: 이력상세/근무자상세,
  본사 쪽 전체)는 별도 논의 후 착수.
- 2026-09-14: 4단계(전체 롤아웃) 진행 — "화면 스캔은 생략하고 배치 단위로
  구현 후 확인·승인받고 다음 배치로" 방식으로 사용자와 합의, 3개 배치로
  나눠 순차 진행(커밋 `59c846a`).

  **배치1(상세형 4개, spacing만)**: 경찰 이력상세, 본사 이력상세·근무자상세·
  경호상세 — 전부 `DetailHeader`+제목/뱃지 줄 구조가 동일해 기계적으로
  적용 가능했음. 경호상세는 `editingBaseInfo` 조건부 렌더가 얽혀있어(폼
  전환 시 제목 줄 자체가 사라짐) wrapper 분리 위치를 신중히 잡아야 했음 —
  `DetailHeader`+조건부 제목줄만 새 `xl:contents` wrapper로 빼고, 폼/본문
  콘텐츠는 기존 바깥 gap-5 div에 그대로 둠.

  **배치2(목록형 6개, 헤더 액션 없음)**: 경찰 이력, 본사 배치요청·연장/
  단축요청(공용 컴포넌트)·관리자계정·경호목록·이력 — 검색+드롭다운 필터
  줄을 `xl` 이상 전용으로 전환. 착수 중 발견: 이 화면들은 상태 구분이
  경호목록의 알약 칩이 아니라 Select 드롭다운이라 모바일에 남길 대체
  UI가 없었음 — 그대로 필터 블록 전체를 데스크톱 전용으로 뺐고(모바일은
  목록 그대로 노출, 필터링 불가), 사용자에게 범위가 넓어진 점을 명시적으로
  확인받아 승인받음. 본사 쪽 3화면의 `SecurityCaseTabs`(경호목록/배치요청/
  연장·단축요청 탭)는 필터가 아니라 네비게이션이라 모바일에서도 유지.

  **배치3(목록형 2개, "+" 액션 있음)**: 경찰 게스트관리, 본사 근무자목록 —
  배치2와 동일 처리 + 제목 옆 모바일 전용 "+" 아이콘 버튼을 각 화면의
  기존 등록/발급 다이얼로그 오픈 핸들러에 연결(신규 페이지 이동이 아니라
  모달이라는 점만 경호목록 파일럿과 다름). 근무자 목록의 "전체 N" 카운트
  뱃지는 칩과 성격이 같다고 판단해 모바일에서도 유지.

  회귀 2건(신규 "+" 아이콘과 기존 텍스트 버튼의 접근성 이름 중복,
  `GuestListPage.test.tsx`·`WorkerListPage.test.tsx`) → 1단계 때 확립한
  `getAllByRole(...)[0]` 패턴으로 동일하게 보정.

  검증: 배치마다 build/lint + 해당 화면 테스트만 우선 재실행, 3배치 전부
  끝난 뒤 `npm run test` 전체 147/147 재확인. 이 세션은 여전히 실백엔드
  네트워크 제약으로 스크린샷 불가 — 사용자가 본인 환경에서 배치마다 직접
  확인 후 승인.

  **다음**: 사용자가 확정 전 확인차 게스트 발급 다이얼로그의 모바일
  스크롤 동작을 질문 — 확인해보니 공용 `components/ui/dialog.tsx`의
  `DialogContent`에 `max-height`/`overflow-y-auto`가 없어 앱 전체
  다이얼로그(18개 파일)가 똑같이 겪는 기존 갭이었음(이번 헤더 작업과는
  무관). 별도로 같이 고치기로 함 — 작업계획: `DialogContent`를 바깥
  (위치/최대높이 `max-h-[85vh]`만 담당)/안쪽(기존 `grid gap-4 p-4` +
  `overflow-y-auto`로 스크롤) 두 겹으로 나누고, 닫기(X) 버튼은 바깥에
  남겨 스크롤 중에도 고정되게 하는 방식(B안)으로 진행 예정 — 다음
  세션/턴에서 착수. 본사 대시보드(목업 없음)·신규접수/수정 폼 화면은
  여전히 범위 밖.
- 2026-09-14: 공용 Dialog 모바일 스크롤 수정 완료(B안, 커밋 `538a32c`).
  `DialogContent`를 바깥(위치+`max-h-[85dvh]`만 담당)/안쪽(기존
  `grid gap-4 p-4` + `overflow-y-auto`+`min-h-0`로 스크롤) 두 겹으로
  분리, 닫기(X) 버튼은 바깥에 남겨 스크롤 중에도 고정. 컴포넌트 1개만
  수정해 앱 전체 다이얼로그 18개에 자동 반영, 개별 다이얼로그 파일은
  무변경. build/lint 통과, test 147/147 통과(DOM이 한 겹 깊어졌지만
  `within(dialog)` 기준 쿼리들이라 영향 없음). 이 세션은 네트워크 제약으로
  스크린샷 불가 — 사용자가 실기기(게스트 발급 다이얼로그, 관리번호
  후보 많은 계정)로 스크롤+X버튼 고정 확인 완료.
- 2026-09-14: 신규접수/수정 폼(`SecurityCaseForm`, 공용) 헤더 간격 정리
  완료(커밋 `adc80d5`) — 확인해보니 이 화면은 이미 breadcrumb+제목+설명이
  한 블록(`mt-1` 마진)으로 붙어있어 새 패턴이 필요 없었고, 다른 상세형
  화면들과 간격 값(`gap-3`)만 통일. 이걸로 4단계(전체 롤아웃) 전 화면
  완료 — **본사 대시보드(목업 없음)만 별도 논의 남음**. build/lint/test
  147/147 통과.
