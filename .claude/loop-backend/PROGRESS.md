...
| — | 신규 | [경찰] 본청/지역청/경찰서 + [본사] | 계정 비밀번호 초기화 | 완료 | (이번 세션) | 로드맵/목업에 없던 신규 요구사항. 2026-09-17 대화로 설계 확정 → 스캐폴딩(A안) → **같은 세션에서 백엔드 실 API(`GetPoliceUserList` 확장+신규 `ResetPassword`) 확인·전환까지 완료**. 화면 4개(`/accounts`, `/guests` 확장, `/admin/managers/police`·`/admin/managers/guests`) + `authStore.userSeq` 배선. build/lint/test(161개) 통과, 브라우저 4계정 실백엔드 검증(SPoliceM1 본청·SPoliceM5 경찰서·StecM1 본사 경찰/게스트탭), 콘솔 에러 0. **2026-09-17 UI 후속 수정 3건**: 본청/지역청 `/accounts`는 게스트 제외(조직계정만, 게스트는 소속 경찰서 소관), 경찰서 계정관리에 "게스트 계정" 소제목 추가(내 계정과 구분), 본부관리자는 관리자 탭 바 자체를 숨김(경찰/게스트 탭이 없어 "본사" 하나뿐인 탭이 의미 없음) — 브라우저(SPoliceM1/SPoliceM5/StecM2) 재검증 완료. 상세는 `CARRYOVER.md` A절(해결 완료 항목) |
...
| 날짜 | 항목 | 상태 | 커밋 | 비고 |
|---|---|---|---|---|
| 2026-09-15 | 모바일 뷰포트 핀치줌(확대) 비활성화 | 완료 | `fa68276` | |
| 2026-09-15 | 폼 필드 모바일 폰트 13px로 통일 | 완료 | `e435207` | 기존 iOS 자동확대 방지용 16px 예외 — 핀치줌 자체가 막혀 불필요해져 제거 |
| 2026-09-15 | 경찰서 로그인 랜딩을 현황으로, 본사 대시보드 메뉴 제외 | 완료 | `7d8f45a` | |
| 2026-09-15 | 본사 경호관리에 상태별 KPI 카드 추가 | 완료 | `31ec0e4` | |
| 2026-09-15 | PWA 아이콘 세트 추가 + 로컬 HTTPS 테스트 환경 구성 | 완료 | `608ae34` | `.certs/`(mkcert 인증서, gitignore) — LAN IP로 휴대폰에서 HTTPS 접속 테스트용 |
| 2026-09-15 | PWA 매니페스트/서비스워커 적용 (Phase 1) | 완료 | `cd10f98` | Android 실기기 검증 완료(2026-09-15), iOS 테스트/IDC 배포/설치 매뉴얼은 사용자가 명시적으로 보류 |
| 2026-09-16 | 파비콘·rail 로고를 safety-link-icon PNG로 통일 | 완료 | `e9b01b5` | |
| 2026-09-16 | 홈 대시보드(경찰) 실 API 연동 | 완료 | `cb2dd38` | 위 번호 표 "홈 대시보드" 행에 부분완료(△)로 반영 완료 |
| 2026-09-16 | 홈 대시보드 모바일 평균경호기간 카드 추가 + 월별추이 y축 0 고정 | 완료 | `72d0dfd` | |
| 2026-09-16 | 목록/상세 화면 스켈레톤 + 액션 로딩 오버레이 적용 | 완료 | `36786a5` | |
| 2026-09-16 | 홈 대시보드 최초 로딩 스켈레톤 적용 | 완료 | `5379628` | |
| 2026-09-16 | 로그인 화면 목업 반영 | 완료 | `129cdcd` | |
| 2026-09-16 | 로그인 화면 input/버튼 높이 확대(h-9→h-11) | 완료 | `5c0e0f8` | 전역 `Input`/`Button` 기본값(목록·필터 화면 밀도 기준)은 유지, 로그인 화면만 클래스 override. 텍스트/placeholder 크기는 원래 그대로 |
| 2026-09-16 | 모바일 터치 active(눌림) 피드백 추가 | 완료 | `860077f` | **원인**: Tailwind v4 preflight가 `-webkit-tap-highlight-color: transparent`를 전역 적용 + 최근 핀치줌 비활성화까지 겹쳐 네이티브 터치 피드백이 전혀 없음. 공용 `Button`엔 `active:translate-y-px`(1px 이동)만 있고 배경 변화 없음. 테이블 행·모바일 카드 5곳·raw `<button>`(필터 칩/탭/FAB)·하단 모바일 nav엔 `active:` 자체가 없음(hover만, 터치에서 신뢰 불가). **적용**: 요소 타입별 `active:` variant 명시(전역 tap-highlight 되살리는 방식 대신 선택, 사용자 결정). 흰 카드 위 요소는 `active:bg-secondary`(slate-200) — 처음 `active:bg-muted` 썼다가 muted(slate-100)가 배경색과 거의 같아 파일럿 스크린샷에서 안 보여서 한 단계 더 진하게 조정. 어두운 solid 배경(기본 Button/FAB/선택된 칩·탭) 위는 `active:bg-primary/70` 대신 `active:bg-slate-700`(밝은 네이비로 확실히 구분). destructive는 `/10→hover/20→active/40`, 하단 nav 아이콘은 `active:scale-90`(Tailwind v4는 네이티브 CSS `scale` 속성 사용, `transform`엔 안 잡힘 — Playwright로 실측 확인). 파일: `button.tsx`·`table.tsx`·`Sidebar.tsx`(공용, 전체 앱 자동 반영) + 경찰/본사 `SecurityCaseListPage`·`HistoryListPage`, 본사 `WorkerListPage`(모바일 카드), 경찰 `GuestListPage`(FAB), 본사 `SecurityCaseTabs`(경호관리 4탭 공용)·경찰 `PeriodRequestDialog`(연장/단축 세그먼트 탭, 계획엔 없었으나 같은 패턴이라 발견 후 포함). 검증: Playwright로 실제 마우스다운 유지 후 `getComputedStyle`/`matches(':active')`로 색상·scale 실측(스크린샷만으로는 `:active`가 순간이라 안 잡힘) + build/lint/test(158개) 통과. Radix 기반(Select/Calendar/DropdownMenu)은 자체 하이라이트 상태가 있어 이번 범위 제외 |
| 2026-09-16 | 폼/경호계획서 등록 화면 모바일 floating nav 숨김 | 완료 | `c4f501d` | 사용자가 UI 디테일 3건(폼 floating nav·이력 필터·테이블 간격) 제안 중 1번 — 경찰 신규접수(`/security-cases/new`)·배치요구서 수정(`/security-cases/:id/edit`)은 별도 라우트, 본사 경호계획서 정보 등록/수정은 URL이 안 바뀌고 `SecurityCaseDetailPage`의 `editingBaseInfo` state로만 토글되는 케이스라 사용자가 직접 포함 여부를 물어 스코프에 추가됨. 구현은 라우트 매칭이 아니라 `shared/hooks/useMobileNavStore.ts`(zustand) 신규 — 페이지가 `useHideMobileNav(hidden)` 훅으로 직접 숨김을 요청하면 `Sidebar.tsx`가 그 store를 구독해 `xl:hidden` 하단 nav를 조건부 렌더. 폼 컴포넌트(`SecurityCaseForm.tsx`)는 상시 `true`, 본사 상세 페이지는 `editingBaseInfo`를 그대로 전달. nav 자리 확보용 하단 여백(`pb-24`/`pb-28`)도 숨김 시 `pb-8`로 축소. 검증 중 `run-s-pgms` 드라이버의 `login-police`/`login-company`가 옛 h1 텍스트("로그인")를 기다리고 있어(로그인 화면 목업 반영 이후 h1이 "Safety Link"로 바뀜, 위 행 참고) 항상 타임아웃나던 걸 발견해 `#login-id` 존재 여부로 수정(스킬 파일, 앱 코드와 무관). build/lint/test(158개) 통과, 실백엔드(SPoliceM5·StecM1) 모바일 뷰포트로 3개 화면 전부 스크린샷 확인(경찰 신규접수·수정 폼은 nav 없음, 본사는 등록 폼 진입 시 사라지고 취소 시 복귀) — 콘솔 에러 없음 |
| 2026-09-16 | 테이블 row 간격을 목업 실측값(14px/20px)에 맞춰 확대 | 완료 | `860a490` | UI 디테일 3건 중 3번. `components/ui/table.tsx`의 `TableCell`이 shadcn 기본 `p-2`(8px)를 그대로 쓰고 있어 목업 원본(`docs/PGMS_UI_mock.dc.html`) `padding:14px 20px`보다 좁았음 — 2026-08-22 타이포그래피 파일럿 때 글자 크기(`text-table-header`/`text-table-body`)만 토큰화하고 패딩은 shadcn 기본값 그대로 남아있었던 게 원인. `TableCell`은 `p-2` → `py-3.5 px-5`(14px/20px)로 목업값 그대로 적용. `TableHead`는 `text-trim` + 고정 높이(`h-10`) 조합이라 패딩 기반 auto-height로 바꾸면 2026-08-27에 겪은 높이 축소 버그(패딩+line-height로 높이가 정해지는 요소에 text-trim 적용 시 leading이 걷히며 높이 자체가 줄어듦, `StatusBadge` 사례) 재발 위험이 있어 높이는 `h-10` 그대로 두고 좌우 패딩만 `px-2`→`px-5`로 확대. 공용 컴포넌트라 앱 전체 테이블(목록·이력·근무자 등)에 자동 반영. 사용자 요청으로 이번엔 스크린샷 생략, 경호목록 화면에서 사용자가 직접 확인 후 "이대로 적용" 승인. build/lint/test(158개) 통과 |
| 2026-09-16 | 이력 조회 필터를 모바일까지 노출 | 완료 | `27fb655` | UI 디테일 3건 중 2번(3건 전부 종결). 경찰(본청/지역청/경찰서)·본사 이력 조회 필터 블록(최종상태·기간·지역청·경찰서·검색)이 `hidden xl:flex`라 모바일엔 필터가 아예 없었음. 처음엔 경호목록처럼 상태 칩만 모바일에 최소 노출하는 안을 제안했는데, 사용자가 "기간이랑 검색 모두 모바일에 필요해"라고 반박 → 지역청/경찰서 선택까지 포함해 필터 전체를 모바일에 노출할지 물어(AskUserQuestion) "전부 노출"로 결정 — 상태 칩 UI를 새로 만드는 대신 기존 필터 블록을 그대로 여는 쪽으로 범위가 단순해짐. 구현: 필터 컨테이너 `hidden gap-2.5 xl:flex xl:flex-wrap xl:items-center` → `flex flex-col gap-2.5 xl:flex-row xl:flex-wrap xl:items-center`(데스크톱 가로 배치 유지, 모바일 세로 스택). 상태/지역청/경찰서 Select·검색 Input은 이미 `w-full ... sm:w-*` 패턴이라 손 안 댐, 기간 `DateField` 두 개만 고정 `w-40` → `flex-1 min-w-0 ... sm:w-40 sm:flex-none`로 모바일에서 균등 분할되도록 조정. 두 파일(경찰/본사 `HistoryListPage.tsx`) 동일 적용. build/lint/test(158개) 통과, 실백엔드(SPoliceM1 본청·SPoliceM5 경찰서·StecM1 본사) 모바일 뷰포트로 3개 role 조합 스크린샷 확인 — 필터 5개(본청/본사)·3개(경찰서, 지역청/경찰서 없음) 전부 줄바꿈·overflow 없이 렌더, 콘솔 에러 없음. **UI 디테일 3건(폼 floating nav·이력 필터·테이블 간격) 전체 완료** |
| 2026-09-17 | 본사 경호계획서 정보 수정 화면 URL 분리 | 완료 | `e6d2a35` | 경찰 `SecurityCaseEditPage.tsx` 패턴 그대로 `/admin/security-cases/:id/edit` 신설(`company/pages/SecurityCaseEditPage.tsx`), `SecurityCaseDetailPage.tsx`의 `editingBaseInfo` state·인라인 `BaseInfoForm` 분기 제거 → "등록"/"수정" 둘 다 `Link`/`navigate`로 이동. `SecurityCaseDetailPage.test.tsx`를 두 라우트(`:id`·`:id/edit`) 등록하는 메모리라우터로 갱신, "경호계획서 정보 등록" 클릭 어서션을 `button`→`link` role로 수정. build/lint/test(158개) 통과. 실백엔드(StecM1, caseSeq 63/ST0019) 검증: 상세→"수정" 클릭 시 URL이 `/admin/security-cases/63/edit`로 분리, 폼 렌더 정상, "취소" 클릭 시 `/admin/security-cases/63`로 정상 복귀, 콘솔 에러 0 |
| 2026-09-17 | 대시보드 상태별 현황 카드 모바일 폰트 확대 | 완료 | `75ba2a7` | `DashboardPage.tsx` 모바일 카드만: LABEL `text-[11px]`→`text-xs`, 아이콘 `size-3`→`size-3.5`, VALUE `text-lg`→`text-xl`, 단위 `text-[11px]`→`text-xs`. 데스크톱(13px/27px)은 그대로(사용자 확인). build/lint/test 통과, 실백엔드(SPoliceM5) 모바일 뷰포트 스크린샷 확인 |
| 2026-09-17 | 대시보드 "이번달 신규접수"/"평균 경호기간" 모바일 flex-row 정렬 | 완료 | `75ba2a7` | 모바일 두 카드의 `CardContent`를 `flex flex-col gap-2`(라벨 위/값 아래)→`flex items-center justify-between`(라벨 좌/값 우 한 줄)로 변경 — 데스크톱은 2026-09-15에 이미 이 패턴이라 모바일만 맞춤. 실백엔드(SPoliceM5) 모바일 스크린샷으로 한 줄 정렬 확인 |
| 2026-09-17 | 대시보드 경찰서 계정 조직계층 패널 제거 | 완료 | `75ba2a7` | `canSelectOrgScope = user?.role !== '경찰서'`(기존 `includeTopOrder`와 동일 조건, 별도 상수로 분리) 추가 — 모바일 조회범위 pill을 경찰서면 `button`(Sheet 트리거) 대신 비인터랙티브 `span`(화살표 아이콘 제거)으로, 모바일 `<Sheet>` 전체와 데스크톱 좌측 `<aside>`(조직 계층)를 `canSelectOrgScope &&`로 조건부 렌더. 실백엔드로 SPoliceM5(경찰서)는 데스크톱에서 좌측 패널이 사라지고 본문이 풀폭으로 렌더됨을 확인, SPoliceM1(본청)은 좌측 패널이 그대로 남아있어 회귀 없음도 함께 확인 |
| 2026-09-17 | 공통 — xl 미만 가로 스크롤 칩/탭 초기 스크롤바 숨김 | 완료 | `7aed93b` | `index.css`에 `@utility scrollbar-hide`(scrollbar-width:none + `::-webkit-scrollbar{display:none}`, 2026-09-15 전역 스타일링 회귀와 달리 이 유틸은 특정 요소에만 적용되는 범위라 안전) 신설 → `police/SecurityCaseListPage.tsx`(상태 필터 칩)·`company/SecurityCaseTabs.tsx`(경호관리 탭) 두 곳에 적용. build/lint/test 통과. **주의**: 마운트 시 순간적으로 보였다 스와이프 후 사라지는 네이티브 오버레이 스크롤바 특성상 스크린샷·헤드리스 자동화로는 수정 효과를 확증하기 어려움 — 실기기(모바일)에서 사용자 육안 확인 필요 |
...
- 2026-09-15: **[본사] 경호계획 조치 5개(`summary1~5`) 인코딩을 콤마 조인 → 비트
  ...
  기준 비트 위치 문자열**("1001")로 저장하는 방식이었음. 단서는 caseSeq 46 —
  2026-09-04에 우리 앱이 콤마 텍스트(`"맞춤형 순찰, CCTV"`)로 써넣었는데, 이후
  재조회 시점엔 `"1100"`으로 바뀌어 있었던 것(우리 앱 경로가 아닌 다른 경로로 덮어써짐).

  안전조치(4개: 맞춤형순찰/임시숙소/스마트워치/CCTV) 비트 순서를 백엔드에서 확인받아
  먼저 등록만 전환·실측(등록 성공, 조회 시 한글로 정상 복원 — 서버가 조회 응답에선
  이미 사람이 읽을 수 있는 값으로 변환해서 준다는 것도 이때 확인돼 `parseMeasureItems`는
  손 안 댐). 이어서 나머지 4개 섹션(긴급응급조치/잠정조치/긴급임시조치/임시조치)도
  등록·수정 양쪽 다 사용자가 실측 확인 완료 — 전 섹션 = 지금 프론트 옵션 배열 순서가
  곧 비트 순서로 확정.

  중간에 수정(`PatchCaseInfo`) 화면에서 "맞춤형 순찰"(프론트, 띄어쓰기 있음) 칩이
  선택 안 된 채로 뜨는 버그 발견 → 서버 공식 표기가 `"맞춤형순찰"`(띄어쓰기 없음)임을
  확인, 프론트 라벨을 교정(`SAFETY_MEASURE_OPTIONS`). 처음엔 등록만 비트로 전환하고
  수정은 콤마 조인 유지하는 절충안으로 갔었는데, 수정 저장 시 비트 데이터가 다시 콤마
  텍스트로 덮어써지는 문제가 있어 사용자 확인 후 수정도 비트로 전환·`toCaseInfoBody`의
  분기 플래그 제거(둘 다 항상 비트).

  구현: `shared/lib/caseMeasures.ts`에 5개 섹션 옵션 배열 export(`SAFETY_MEASURE_OPTIONS`
  등, `BaseInfoForm.tsx` 로컬 배열 제거해 공유) + `joinMeasureItemsAsBits` 신설.
  `features/company/api/securityCaseDetail.ts::toCaseInfoBody`가 등록·수정 둘 다 이
  인코더 사용.

  검증: `npm run test` 150/150(신규 3건) · lint(기존 warning 2) · build 통과. 실백엔드는
  사용자가 직접 등록/수정 폼에서 5개 섹션 전부 선택→저장→재조회 왕복 확인(피전·본사
  양쪽 조회 화면 모두 한글로 정상 표시).

  **문서**: `findings.md` #11 🟡→🟢(전체 종결), `matrix.md` #9 손실 매핑 각주 갱신.
  **다음**: 다른 작업.
...
- 2026-09-14: **회신 반영 — `GetGuardCaseList` 담당자 `userSeq` 조인**(findings #1 완전
  ...
  매칭(`assigneeName`)을 쓰고 있어 반영.

  구현: `features/company/api/requests.ts`의 `GuardCaseRow`에 `userSeq: number | null`
  추가, `guardCaseRowToSecurityCase`가 `assigneeId: String(row.userSeq)`로 매핑(기존
  `SecurityCase.assigneeId` 필드 재사용 — 이미 mock 경로에서 쓰던 id 축). 이름 매칭이던
  두 곳을 id 매칭으로 전환: `ManagerAccountListPage.assignedCountOf`(배정건수 열),
  `ManagerAssignedCasesDialog`(담당경호 목록) — 둘 다 `c.assigneeId === String(account.userSeq)`.
  테스트 더블(`mocks/handlers/guardCase.ts`)도 `userSeq: c.assigneeId ? userSeqOf(c.assigneeId)
  : null` 추가해 실제 매핑 로직까지 오프라인 테스트가 검증하게 함(기존 `userSeqOf` 헬퍼
  재사용 — `GetStecUserList` 더블이 이미 같은 방식으로 `userSeq` 합성 중이었음).

  검증: `npm run test` 144/144(회귀만, 신규 테스트 없음 — 기존 테스트가 id 매칭 경로를
  이미 타고 있어 로직 전환이 그대로 검증됨) · lint(기존 warning 2) · build 통과. 실백엔드
  (`StecM1`) 브라우저 검증: `/admin/managers`에서 HS2본부(userSeq 104) 배정건수 10건,
  ⋮ → 담당경호 다이얼로그도 10건(caseSeq 29·52~58·61·62, curl로 미리 확인한 목록과
  정확히 일치) — 콘솔 에러 0.

  **문서**: `findings.md` #1 🟡→🟢(전체 종결로 헤더 변경, userSeq 조인 해결 서술 추가),
  `CARRYOVER.md` A절 갱신(#1 완전 종료, 남은 건 `isRepresentative`뿐).
...
- 2026-09-11: **URL 직접 접근 스코프 일괄 테스트 (CARRYOVER C절) — 전부 소진**. 새 화면
  ...
  회귀 재확인) 추가.
  - `local/_probe-url-scope.sh` 신규 — 0단계로 강남경찰서(타 관할) caseSeq/deployReqSeq,
    게스트(SPoliceGuest3) 조회권 없는 deployReqSeq를 먼저 조회해 대상 ID 확보.
  - 5건 전부 **차단 확인**: `History/Police/GetHistoryDetail`(SPoliceM3→타관할 404, 대조군
    SPoliceM1 200) / `History/Stec/GetHistoryDetail`(StecM3→남 소유 404) /
    `Deploy/Police/GetDeployDetail`(게스트→조회권 없음 404, 대조군 SPoliceM5 200) /
    같은 EP(SPoliceM3→타관할 404, 대조군 SPoliceM1 200) / `GuardCase/Stec/GetGuardCaseDetail`
    (StecM3→남 소유 403, 2026-09-08과 동일 메시지 — 회귀 없음, **#12 완료 전환**).
    피전 URL 직접 접근(접수 상태)도 브라우저로 확인 — "접수취소"만 노출, 배정 이후
    액션 없음. loop-screens Phase 1 #3 이월분도 아키텍처상 도달 불가로 함께 해소.
  - 대조군(정상 접근 계정으로 같은 ID 호출 → 200)을 같이 돌려서 404가 파라미터 오류가
    아니라 진짜 스코프 차단인 걸 구분(1·3·4번은 처음 캐치 없이 바로 404만 보면 판단이
    애매해 대조군이 필수였음).
  - **부수 발견(조치 불필요)**: `History/Police/GetHistoryDetail`이 이제 종결/취소 아닌
    상태(배정·경호중·경호완료)에도 200을 준다 — 예전 "History는 종결/취소만" 문서와
    다름. 화면이 이 경로로 안 불러 영향 없음, 참고 기록만.
  - 코드 변경 없음(전부 GET, 상태 안 바꿈) — `npm run test`/lint/build 재실행 불필요.
  - **문서**: CARRYOVER C절 전부 소진 표시, D절 아님(별도 섹션), loop-backend #12
    "검증완료·문서보류"→완료, loop-screens Phase 1 #3 미해결 항목 해소.
  - **다음**: 커밋 후 — 남은 건 백엔드 회신 대기(A절)뿐, 실행 가능한 새 프론트 작업 없음.
...
- 2026-09-08: 17번([경찰서] 게스트 · 경호목록 + 상세 조회전용) — **완료**. 그룹 D
  ...
  - **미검증(이월, 2026-09-14 소진)**: 게스트가 조회권 없는 *진행중* 건 상세를 직접 호출 시
    403/404인지 — 당시엔 동래에 활성 미부여 건이 없어(활성 1건, 게스트가 봄) 양성 테스트
    불가했음. 이후 사용자가 SPoliceGuest4(조회권=deploySeq 100=ST0017)로 조회권 없는
    deploySeq 92(ST0010)에 URL 직접 접근해 **차단 확인** — `GetDeployList` groupSeq 무시
    동작과 일치, 추정이 아니라 실측으로 종료.
