...
| 2 | A | [경찰서] 피전 | 경찰서 경호목록 | 완료 | `2679751` | 스코프는 서버가 403으로 강제(analysis.md 4-6 해소). 3번 직후 재검증 완료(새 접수 반영·mgmtNo 조합형태 확인). **배정 이후 상태 문자열은 여전히 미검증 → 9번(본사 경호 상세) 이후 재검증** |
...
| 5 | A | [경찰서] 피전 | 배치요구서 수정 | 대기 | | ← **다음 대상**. `GetDeployDetail` prefill 필드 부족(성별·생년월일·직업 등, exclusions `경호 상세`) 어디서 받을지 먼저 확인. **여기까지가 피전 경호관리 섹션** — 종료 시 issues(#5·#6 등)/exclusions 정리해 백엔드 일괄 요청 |
| 6 | B | [본사] 운영/시스템관리자 | 근무자 목록/등록 | 대기 | | 9번(경호계획 등록)의 선행 의존성 |
