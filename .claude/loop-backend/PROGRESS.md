...
| — | 신규 | [경찰] 본청/지역청/경찰서 + [본사] | 계정 비밀번호 초기화 | 부분완료(△) | (이번 세션) | 로드맵/목업에 없던 신규 요구사항. **2026-09-17 대화로 설계 확정 후 스캐폴딩(A안) 완료** — 화면 4개(`/accounts`, `/guests` 확장, `/admin/managers/police`·`/admin/managers/guests`) + `authStore.userSeq` 배선. build/lint/test(161개) 통과, 브라우저 검증(①③ MSW 더블·②는 실백엔드 SPoliceM5). **△ 사유**: 엔드포인트 경로가 전부 임시 추정치(`GetPoliceAccountTree`·`ResetPoliceAccountPassword`) — 백엔드 회신 오면 실제 스펙으로 교체하고 더블을 `testOnlyHandlers`로 이관해야 완료. 상세는 `CARRYOVER.md` A절 |
