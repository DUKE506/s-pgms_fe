...
| 2026-09-17 | 본사 경호계획서 정보 수정 화면 URL 분리 | 완료(커밋 전) | | 경찰 `SecurityCaseEditPage.tsx` 패턴 그대로 `/admin/security-cases/:id/edit` 신설(`company/pages/SecurityCaseEditPage.tsx`), `SecurityCaseDetailPage.tsx`의 `editingBaseInfo` state·인라인 `BaseInfoForm` 분기 제거 → "등록"/"수정" 둘 다 `Link`/`navigate`로 이동. `SecurityCaseDetailPage.test.tsx`를 두 라우트(`:id`·`:id/edit`) 등록하는 메모리라우터로 갱신, "경호계획서 정보 등록" 클릭 어서션을 `button`→`link` role로 수정. build/lint/test(158개) 통과. 실백엔드(StecM1, caseSeq 63/ST0019) 검증: 상세→"수정" 클릭 시 URL이 `/admin/security-cases/63/edit`로 분리, 폼 렌더 정상, "취소" 클릭 시 `/admin/security-cases/63`로 정상 복귀, 콘솔 에러 0 |
| 2026-09-17 | 대시보드 상태별 현황 카드 모바일 폰트 확대 | 완료(커밋 전) | | `DashboardPage.tsx` 모바일 카드만: LABEL `text-[11px]`→`text-xs`, 아이콘 `size-3`→`size-3.5`, VALUE `text-lg`→`text-xl`, 단위 `text-[11px]`→`text-xs`. 데스크톱(13px/27px)은 그대로(사용자 확인). build/lint/test 통과, 실백엔드(SPoliceM5) 모바일 뷰포트 스크린샷 확인 |
| 2026-09-17 | 대시보드 "이번달 신규접수"/"평균 경호기간" 모바일 flex-row 정렬 | 완료(커밋 전) | | 모바일 두 카드의 `CardContent`를 `flex flex-col gap-2`(라벨 위/값 아래)→`flex items-center justify-between`(라벨 좌/값 우 한 줄)로 변경 — 데스크톱은 2026-09-15에 이미 이 패턴이라 모바일만 맞춤. 실백엔드(SPoliceM5) 모바일 스크린샷으로 한 줄 정렬 확인 |
| 2026-09-17 | 대시보드 경찰서 계정 조직계층 패널 제거 | 완료(커밋 전) | | `canSelectOrgScope = user?.role !== '경찰서'`(기존 `includeTopOrder`와 동일 조건, 별도 상수로 분리) 추가 — 모바일 조회범위 pill을 경찰서면 `button`(Sheet 트리거) 대신 비인터랙티브 `span`(화살표 아이콘 제거)으로, 모바일 `<Sheet>` 전체와 데스크톱 좌측 `<aside>`(조직 계층)를 `canSelectOrgScope &&`로 조건부 렌더. 실백엔드로 SPoliceM5(경찰서)는 데스크톱에서 좌측 패널이 사라지고 본문이 풀폭으로 렌더됨을 확인, SPoliceM1(본청)은 좌측 패널이 그대로 남아있어 회귀 없음도 함께 확인 |
| 2026-09-17 | 공통 — xl 미만 가로 스크롤 칩/탭 초기 스크롤바 숨김 | 완료(커밋 전) | | `index.css`에 `@utility scrollbar-hide`(scrollbar-width:none + `::-webkit-scrollbar{display:none}`, 2026-09-15 전역 스타일링 회귀와 달리 이 유틸은 특정 요소에만 적용되는 범위라 안전) 신설 → `police/SecurityCaseListPage.tsx`(상태 필터 칩)·`company/SecurityCaseTabs.tsx`(경호관리 탭) 두 곳에 적용. build/lint/test 통과. **주의**: 마운트 시 순간적으로 보였다 스와이프 후 사라지는 네이티브 오버레이 스크롤바 특성상 스크린샷·헤드리스 자동화로는 수정 효과를 확증하기 어려움 — 실기기(모바일)에서 사용자 육안 확인 필요 |
