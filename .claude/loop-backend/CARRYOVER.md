...
| ~~[신규] 계정 비밀번호 초기화 API 2종~~ (2026-09-17, 사용자가 세션 밖에서 직접 요청) | 목록조회 + 초기화 | **✅ 전부 해결·실 API 전환 완료(2026-09-17).** 목록조회는 신규 EP가 아니라 기존 `User/Stec/W/GetPoliceUserList`가 확장된 것(설명에 `guestList` 필드 추가) — 실측 확인(응답 샘플 `responses/User-Stec-GetPoliceUserList.md`): 본청 호출 시 `data[0]`이 본청 자신(레벨1)이라 "본인 포함" 요구사항이 API 차원에서 이미 해결돼 있었음, 게스트는 `children`이 아니라 노드별 `guestList` 필드(`codeSeq:7`)로 붙음. 초기화는 신규 `PATCH User/Stec/W/ResetPassword`(body `{userSeq}`만) — 실측 확인(`responses/User-Stec-ResetPassword.md`, SPoliceM5가 자기 소속 게스트 SPoliceGuest3 대상으로 실제 호출→`pwChangedYn` false→true 반영 재조회 확인). `features/police/api/accountManagement.ts`를 실제 경로로 교체(`GetPoliceAccountTree`·`ResetPoliceAccountPassword` 임시 경로 → 위 실경로), `flatten()`이 `guestList`를 별도로 펼치도록 수정. mock(`policeAccountTree.ts`+`policeAccounts.ts`)은 실 연동 완료 화면 관례대로 `testOnlyHandlers`로 이관(브라우저는 실백엔드). build/lint/test(161개)·브라우저 4계정(SPoliceM1 본청/SPoliceM5 경찰서/StecM1 본사 경찰·게스트탭) 실백엔드 검증 완료, 콘솔 에러 0 |
